...
const app = express();
const { Todo, sequelize } = require("./models"); // Import sequelize instance from models
...
// 🔁 Redirect from root to /todo-app
app.get("/", function (request, response) {
  response.redirect("/todo-app");
});

// 🧩 Landing route (you may replace this with actual content or EJS view)
app.get("/todo-app", function (request, response) {
  response.render("index"); // or any other view you prefer
});
...
// Sync the database first, then start the server
sequelize.sync({ force: false })  // force: false means it will not drop tables, only create if missing
  .then(() => {
